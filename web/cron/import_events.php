...
$sql = "SELECT id FROM ImportTemporary_Event WHERE inserted != 'y' AND import_log_id = " . $last_import_log;
...
if (!$import_stop) {
    ...
    if (mysql_num_rows($result) > 0) {
            ...
            if ($row["error"] == "n" || empty($row["error"])) {
                        ...
                        if ($current_category_tree) {
                            foreach ($current_category_tree as $current_category) {
                                if ($first_category) {
                                    ...
                                    $sql_category_id = "category_id = " . ($last_category_id);
                                }
                                $sqlCategory = "SELECT * FROM EventCategory WHERE $sql_category_id AND title = " . db_formatString($current_category, "", true) . "";
                                ...
                                if (mysql_num_rows($resultCategory) <= 0) {
                                    ...
                                    $sqlCategory = "INSERT INTO EventCategory (title, page_title, category_id, featured, friendly_url, enabled) VALUES (" . db_formatString($current_category, "", true) . ", " . db_formatString($current_category, "", true) . ", " . ($first_category ? "NULL" : $last_category_id) . ", " . ($import_featured_categs == "y" ? db_formatString("y") : db_formatString("n")) . ", " . db_formatString($category_friendly_url) . ", 'y')";
                ...
                if ($row["event_title"]) {
                    ...
                    foreach ($allLevels as $eachLevelValue => $eachLevelName) {
                        if ($row["event_level"]) {
                            if (string_strtoupper($eachLevelName) == string_strtoupper($row["event_level"])
                            ) {
                    ...
                    if ($categoryIDArray) {
                        ...
                        foreach ($categoryIDArray as $category_id_tree) {
                            ${"cat_" . $count_cat_aux . "_id"} = ($category_id_tree[count($category_id_tree) - 1] ? $category_id_tree[count($category_id_tree) - 1] : 'NULL');
                    ...
                    if ($needToUpdate) {
                        ...
                        $sqlEvent .= "account_id = " . ($account_id ? db_formatNumber($account_id) : 'NULL') . ", ";
        ...
        log_addCronRecord($link, "import_events", $messageLog, true, $cron_log_id);
        $sql = "SELECT id FROM ImportTemporary_Event WHERE import_log_id = '" . $importlog_id . "' AND inserted != 'y' ORDER BY id LIMIT 1";
        $result = mysql_query($sql, $linkDomain);
        if (mysql_num_rows($result) <= 0) {
            if ($status == "R") {
...
if( $eventsWereUpdated ){
    SymfonyCore::getContainer()->get("event.category.synchronization")->generateAll();
